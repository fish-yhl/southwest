...
def reconcile_countries_by_name(plot_countries, gdp_countries): #返回在世行有GDP数据的绘图库国家代码字典，以及没有世行GDP数据的国家代码集合
	...
	for i in plot_countries:
		for j in gdp_countries.values():
			if plot_countries[i]==j["Country Name"]:
				for years in range(1960,2016):
					if j[str(years)]=='':       #如果有一年GDP不为空，则加入字典
						continue
					else:
						a[i]=j
	for i in plot_countries:                     #字典中的为无GDP记录的国家
		if i not in a:
			b.add(i)
	return (a,b)
...
def build_map_dict_by_name(gdpinfo, plot_countries, year):
	...
	"""
	a=reconcile_countries_by_name(plot_countries,read_csv_as_nested_dict("isp_gdp.csv","Country Code",",",'"'))
	b={}
	c=set()
	d=set()
	e=set()
...
def render_world_map(gdpinfo, plot_countries, year, map_file): #将具体某年世界各国的GDP数据(包括缺少GDP数据以及只是在该年缺少GDP数据的国家)以地图形式可视化
	...
	worldmap_chart.title = "全球GDP分布图"                                                                   #键入标题
